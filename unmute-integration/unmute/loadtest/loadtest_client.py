    ...
    try:
        async for message_raw in websocket:
            ...
            if isinstance(message, ora.ResponseCreated):  # start
                ...
                user_stopwatch.time_phase_if_not_started("text_start", check_previous=False)
...
def main(
    ...
    for audio_file in list(audio_dir.glob("*.mp3")) + list(audio_dir.glob("*.wav")):
        ...
        turn_taking_file = audio_file.parent / "turn_taking.json"
        if turn_taking_file.exists():
            with turn_taking_file.open() as f:
                turn_taking = json.load(f)
            if turn_taking:
                end_sample = int(float(turn_taking[0]["timestamp"][1]) * SAMPLE_RATE)
                audio_file_data = audio_file_data[:end_sample]
